...
import { Share2, Save, ArrowLeft, Eye, X, Wifi, ChevronUp, ChevronDown } from "lucide-react-native";
import { useLocalSearchParams, router } from "expo-router";
import { useApp } from "@/providers/AppProvider";
import { ColorComposition } from "@/types";
...
export default function AnalysisScreen() {
  ...
  // Safely parse analysis data with fallback
  let analysis;
  try {
    analysis = analysisData ? JSON.parse(analysisData as string) : null;
  } catch (error) {
    console.error('Failed to parse analysis data:', error);
    analysis = null;
  }
  
  // If no analysis data, show loading or error state
  if (!analysis) {
    return (
      <View style={[styles.container, { backgroundColor: isDarkMode ? '#1a1a3e' : '#f8fafc' }]}>
        <SafeAreaView style={styles.safeArea}>
          <View style={styles.errorContainer}>
            <Text style={[styles.errorTitle, { color: isDarkMode ? '#fff' : '#1f2937' }]}>Analysis Not Available</Text>
            <Text style={[styles.errorMessage, { color: isDarkMode ? '#a0a0b8' : '#6b7280' }]}>Unable to load analysis data. Please try again.</Text>
            <TouchableOpacity style={styles.analyzeButton} onPress={() => router.replace('/home')}>
              <ArrowLeft size={18} color="#ffffff" />
              <Text style={styles.analyzeButtonText}>Go Back</Text>
            </TouchableOpacity>
          </View>
        </SafeAreaView>
      </View>
      ...
      <SafeAreaView style={styles.safeArea}>
        ...
        >
          ...
          <View style={[styles.imageCard, { backgroundColor: isDarkMode ? '#2a2a4a' : '#ffffff' }]}>
            ...
            {analysis?.dominantColor?.name ? (
              <>
                <Text style={[styles.dominantColorName, { color: isDarkMode ? '#1f2937' : '#1f2937' }]}>{analysis.dominantColor.name}</Text>
                <Text style={[styles.dominantColorConfidence, { color: isDarkMode ? '#6b7280' : '#6b7280' }]}>Dominant Color ({analysis.dominantColor.confidence || 0}% confidence)</Text>
              </>
            ) : (
              <>
                ...
                {analysis.colorComposition.map((color: ColorComposition, index: number) => (
                  <View key={index} style={styles.colorItem}>
                    ...
                    <View style={styles.colorInfo}>
                      <Text style={[styles.colorName, { color: isDarkMode ? '#fff' : '#1f2937' }]}>{color?.name || 'Unknown Color'}</Text>
                      <Text style={[styles.colorHex, { color: isDarkMode ? '#a0a0b8' : '#6b7280' }]}>{color?.hex || '#000000'}</Text>
                    </View>
                    <Text style={[styles.colorPercentage, { color: isDarkMode ? '#fff' : '#1f2937' }]}>{color?.percentage || 0}%</Text>
                  </View>
                ))}
              </View>
            </View>
          ...
          <View style={styles.healthSection}>
            ...
            {/* Iris Texture Smoothness */}
            <TouchableOpacity 
              style={[styles.healthIndicatorCard, { backgroundColor: isDarkMode ? '#2a2a4a' : '#ffffff' }]}
              onPress={() => toggleIndicator('texture')}
            >
              <View style={styles.healthIndicatorHeader}>
                <View style={styles.healthIndicatorLeft}>
                  <View style={styles.healthIndicatorIcon}>
                    <Text style={styles.healthIndicatorEmoji}>○</Text>
                  </View>
                  <View style={styles.healthIndicatorInfo}>
                    <Text style={[styles.healthIndicatorTitle, { color: isDarkMode ? '#fff' : '#1f2937' }]}>Iris Texture Smoothness</Text>
                ...
                <View style={styles.healthIndicatorChevron}>
                  {expandedIndicators.texture ? 
                    <ChevronUp size={20} color={isDarkMode ? '#8a8aa0' : '#6b7280'} /> :
                    <ChevronDown size={20} color={isDarkMode ? '#8a8aa0' : '#6b7280'} />
                  }
                </View>
              </View>
              
              {expandedIndicators.texture && (
                <View style={styles.healthIndicatorDetails}>
                  <Text style={[styles.healthIndicatorDescription, { color: isDarkMode ? '#a0a0b8' : '#4b5563' }]}>
                    A smooth and consistent iris texture, as seen here, is often playfully associated with a feeling of general well-being and a calm inner state.
                  </Text>
                  <View style={styles.comparisonSection}>
                    <Text style={[styles.comparisonTitle, { color: isDarkMode ? '#8a8aa0' : '#6b7280' }]}>Comparison to Average</Text>
                    <Text style={[styles.comparisonText, { color: isDarkMode ? '#a0a0b8' : '#4b5563' }]}>
                      This finding is considered &apos;Normal&apos; and aligns with the average baseline, indicating a balanced state for this particular feature.
                    </Text>
                  </View>
                </View>
              )}
            ...
            {/* Subtle Radial Furrows */}
            <TouchableOpacity 
              style={[styles.healthIndicatorCard, { backgroundColor: isDarkMode ? '#2a2a4a' : '#ffffff' }]}
              onPress={() => toggleIndicator('furrows')}
            >
              <View style={styles.healthIndicatorHeader}>
                <View style={styles.healthIndicatorLeft}>
                  <View style={styles.healthIndicatorIcon}>
                  ...
                  <View style={styles.healthIndicatorInfo}>
                    <Text style={[styles.healthIndicatorTitle, { color: isDarkMode ? '#fff' : '#1f2937' }]}>Subtle Radial Furrows</Text>
                    <View style={[styles.healthStatus, { backgroundColor: isDarkMode ? '#3a3a5a' : '#f3f4f6' }]}>
                      <Text style={[styles.healthStatusText, { color: isDarkMode ? '#8a8aa0' : '#6b7280' }]}>NORMAL</Text>
                    </View>
                  </View>
              ...
              {expandedIndicators.furrows && (
                <View style={styles.healthIndicatorDetails}>
                  <Text style={[styles.healthIndicatorDescription, { color: isDarkMode ? '#a0a0b8' : '#4b5563' }]}>
                    The fine, subtle lines radiating outward from the pupil area might whimsically suggest an active inner energy system, often seen in individuals with a dynamic approach to life.
                  </Text>
                  <View style={styles.comparisonSection}>
                    <Text style={[styles.comparisonTitle, { color: isDarkMode ? '#8a8aa0' : '#6b7280' }]}>Comparison to Average</Text>
                    <Text style={[styles.comparisonText, { color: isDarkMode ? '#a0a0b8' : '#4b5563' }]}>
            ...
            <TouchableOpacity 
              style={[styles.healthIndicatorCard, { backgroundColor: isDarkMode ? '#2a2a4a' : '#ffffff' }]}
              onPress={() => toggleIndicator('pigment')}
            >
              <View style={styles.healthIndicatorHeader}>
                <View style={styles.healthIndicatorLeft}>
                  <View style={styles.healthIndicatorIcon}>
                  ...
                  <View style={styles.healthIndicatorInfo}>
                    <Text style={[styles.healthIndicatorTitle, { color: isDarkMode ? '#fff' : '#1f2937' }]}>Pigment Consistency</Text>
                    <View style={[styles.healthStatus, { backgroundColor: isDarkMode ? '#3a3a5a' : '#f3f4f6' }]}>
                      <Text style={[styles.healthStatusText, { color: isDarkMode ? '#8a8aa0' : '#6b7280' }]}>NORMAL</Text>
                    </View>
                  </View>
              ...
              {expandedIndicators.pigment && (
                <View style={styles.healthIndicatorDetails}>
                  <Text style={[styles.healthIndicatorDescription, { color: isDarkMode ? '#a0a0b8' : '#4b5563' }]}>
                    The even distribution of rich brown pigment hints at a balanced inner landscape, suggesting a steady and consistent nature that is well-rooted.
                  </Text>
                  <View style={styles.comparisonSection}>
                    <Text style={[styles.comparisonTitle, { color: isDarkMode ? '#8a8aa0' : '#6b7280' }]}>Comparison to Average</Text>
                    <Text style={[styles.comparisonText, { color: isDarkMode ? '#a0a0b8' : '#4b5563' }]}>
